...
export class DeepSeekCaller {
  ...
  // Produces realistic, varied output so the system is fully functional.
  // ---------------------------------------------------------------------------
  ...
  private async simulateResponse(
    ...
    // Determine what kind of response to generate based on system prompt content
    const sysLower = system.toLowerCase();
    const userLower = user.toLowerCase();

    let content = '';

    if (sysLower.includes('journal') || userLower.includes('journal')) {
      content = this.simulateJournal(user);
    } else if (sysLower.includes('creative') || sysLower.includes('write a creative')) {
      content = this.simulateCreative(user);
    } else if (sysLower.includes('tap') || sysLower.includes('bar') || sysLower.includes('open mic')) {
      content = this.simulateTap(user);
    } else if (sysLower.includes('onboarding') || sysLower.includes('dear tomorrow')) {
...
Tomorrow's me won't remember the具体的 taste of this.
...
I'll be at the bar if anyone wants the long version. *sets the journal on the counter, open to today's page*`;
    ...
    if (user.toLowerCase().includes('reply') || user.toLowerCase().includes('respond')) {
...
I mapped what I found. Tomorrow should go deeper into the rooms I only glanced through.`;
  ...
  private simulateGeneric(user: string): string {
    // Use a hash of the user input to create deterministic but varied output
    let hash = 0;
